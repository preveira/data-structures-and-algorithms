...
const screenForNames = (arr) => {
  return arr.filter(name => /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z\s]+$/.test(name.trim()));
};

describe('Testing challenge 1', () => {
  test('It should return a list of names', () => {
    const names = ['Mr. Brown', ' Ms. Red', 'Dr. Blue', 'Mrs. Green', '123', 'John Doe'];
    expect(screenForNames(names)).toStrictEqual(['Mr. Brown', 'Ms. Red', 'Dr. Blue', 'Mrs. Green']);
  });

  test('It should return false if no names match the criteria', () => {
    const names = ['John', 'Jane', '123', 'John Doe'];
...
const detectTicTacToeWin = (board) => {
  const helpCheck = (row1, col1, row2, col2, row3, col3) => {
    return board[row1][col1] !== '' && board[row1][col1] === board[row2][col2] && board[row1][col1] === board[row3][col3];
  };
